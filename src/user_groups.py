...
def list_groups(url, token, name=None, page_size=50):
    ...
    while True:
        ...
        if r.status_code != 200:
            raise ScriptError(f"Error: HTTP {r.status_code} - {r.text}")
        res = r.json()
        if "error" in res:
            raise ScriptError(f"Error: {res['error']}")
        rows = res.get("data", [])
        data.extend(rows)
...
def update_group(url, token, name, new_name=None, note=None, accessed_from=None, access_to=None):
    ...
    if not g:
        raise ScriptError(f"Error: Group '{name}' not found")
    guid = g.get("guid")
    payload = {}
...
def delete_groups(url, token, names):
    ...
    for n in names:
        ...
        if not g:
            raise ScriptError(f"Error: Group '{n}' not found")
        guid = g.get("guid")
...
def add_users(url, token, group_name, user_names):
    ...
    if not g:
        raise ScriptError(f"Error: Group '{group_name}' not found")
    guid = g.get("guid")
...
def main(argv=None):
    ...
    if args.command == "view":
        ...
        if not args.name:
            raise ScriptError("Error: --name is required (supports comma separated)")
        ...
        if not args.name or not args.users:
            raise ScriptError("Error: --name and --users are required")
