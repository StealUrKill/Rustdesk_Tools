    ...
    if response.text and response.text.strip():
        try:
            ...
            if isinstance(json_data, dict) and "error" in json_data:
                raise ScriptError(f"Error: {json_data['error']}")
            return json_data
        except ValueError:
...
def enable_strategy(url, token, name):
    ...
    if not strategy:
        raise ScriptError(f"Error: Strategy '{name}' not found")
...
def assign_strategy(url, token, strategy_name, peers=None, users=None, device_groups=None):
    ...
    if strategy_name:
        ...
        if not strategy:
            raise ScriptError(f"Error: Strategy '{strategy_name}' not found")
    ...
    if peers:
        for peer in peers:
            ...
            if len(peer) == 36 and peer.count('-') == 4:
                ...
                if not guid:
                    raise ScriptError(f"Error: Device '{peer}' not found")
    ...
    if users:
        for user in users:
            ...
            if len(user) == 36 and user.count('-') == 4:
                ...
                if not guid:
                    raise ScriptError(f"Error: User '{user}' not found")
    ...
    if device_groups:
        for dg in device_groups:
            ...
            if len(dg) == 36 and dg.count('-') == 4:
                ...
                if not guid:
                    raise ScriptError(f"Error: Device group '{dg}' not found")
...
def main(argv=None):
    ...
    if args.command == "list":
        ...
        if args.guid:
            ...
            if not strategy:
                raise ScriptError(f"Error: Strategy '{args.name}' not found")
            # Get full details by GUID
            guid = strategy.get("guid")
            res = get_strategy_by_guid(args.url, args.token, guid)
            print(json.dumps(res, indent=2))
        else:
            raise ScriptError("Error: --name or --guid is required for view command")
    
    elif args.command == "enable":
        if not args.name:
            raise ScriptError("Error: --name is required")
        ...
        if not args.peers and not args.users and not args.device_groups:
            raise ScriptError("Error: at least one of --peers, --users, or --device-groups is required")
