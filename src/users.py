...
def check_response(response):
    ...
    Raises ScriptError if there's an error.
    """
...
def main(argv=None):
    ...
    if args.command == "new":
        if not args.name or not args.password or not args.group_name:
            raise ScriptError("Error: --name and --password and --group_name are required for new command")
    ...
    if args.command == "invite":
        if not args.email or not args.name or not args.group_name:
            raise ScriptError("Error: --email and --name and --group_name are required for invite command")
    ...
    if args.command == "view":
        ...
        if args.command == "disable":
            ...
            if not args.web_console_url:
                raise ScriptError("Error: --web-console-url is required for enable-2fa-enforce")
